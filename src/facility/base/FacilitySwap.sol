...
abstract contract FacilitySwap is IFacilitySwap, EIP712, ReentrancyGuardTransient {
    ...
    nonReentrant
  {
    ...
    Intent storage intent1 = _facilityStorage.getIntent(params.id1);
    Intent storage intent2 = _facilityStorage.getIntent(params.id2);
    ...
    LibIntent.swap(
      intent1, params.id1, intent2, params.id2, params.token1, params.amount1, params.token2, params.amount2
    );
  ...
  /// @dev Checks if the given address is authorized to sign swaps.
  ///      Must revert if the signer is not authorized.
  /// @param signer The address to validate.
  function _checkSigner(address signer) internal view virtual;
}
