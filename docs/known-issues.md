...
- **The signer address is not part of the digest.** A smart-contract-wallet guardian must ensure its EIP-1271 validation binds its own address (Safe >= 1.3.0 does); wallets sharing an EOA signer can otherwise replay each other's approvals. Wallets with faulty fallbacks (notably some older EIP-4337 fallback handlers that forward `isValidSignature` straight to the underlying EOA signer) must not be quorum signers (CS Note 10.14).
...
- **Approve amounts in `[2^128, 2^256-2]` become infinite.** PT/YT allowances are stored as uint128 with `uint256.max` as the infinite sentinel, which is never decremented on `transferFrom`; values in between are rejected or normalized rather than tracked exactly (CS Note 10.2).
...
- **Empty-vault redemption never reverts.** Once withdrawals are permitted, `convertToAssets` prices shares with `mulDiv` even when the request holds zero assets, so `burnAll` burns the shares and transfers nothing rather than reverting; the initial-conversion fallback (PT 1:1, YT 0) applies only while the token supply is zero, or while assets are zero and withdrawals are still locked (C 3.2.4 / CS-GRUNT-014).
...
Fee accounting around bad debt has documented edges: the management-fee basis drops a module's entire collateral the wei it crosses into bad debt (CFR 3.1.3); the `lastDebt == 0` bootstrap sentinel makes the next accrual skip the performance fee and reseed, and it can be reached through full debt repayment, the rebase carry clamp, or an owner reset during a full bad-debt episode (CFR 3.1.2; all-bad-debt accruals themselves hold the pre-episode reference, see [Position Manager](position-manager.md#the-reference-as-a-high-water-mark)); and `totalDebt`/`lastDebt` are rounded-down accounting values sourced from `totalBorrowed()`, never to be reused for solvency or liquidation logic (CFR 3.1.4).
...
**Queues and rebalancing.** Deposit collateral not needed to back a borrow, and the whole deposit when no debt is requested, routes to `supplyQueue[0]`, relying on Morpho collateral supply being uncapped; a future capped `IBorrowPosition` would need different routing, and curators rebalance after deposits when distribution matters (C 3.3.7). `maxRebalanceLoss` must be nonzero in production: Morpho rounds repay shares down and borrow shares up, a 1-2 wei phantom NAV decrease per rebalance even with no economic loss, which a single basis point absorbs. The loss check compares oracle-dependent NAV, so a mid-rebalance price move can mask real loss (CS-GRUNT-102), and adding or removing a non-flat module steps NAV in one transaction, which is sandwichable (C 3.3.17). A dominant Morpho lender can manipulate deposit routing by moving liquidity around Grunt transactions (CS Note 10.19).
...
- **A Request past its 90-day deadline auto-expires** and bypasses the retargetter's repayment gates: `repay()` reverts with `AlreadyRepaid` and late remediation is arranged off-chain, deliverable only through the beacon upgrade path (multisig + timelock). Operations are expected to settle far inside the horizon; if 90 days proves too short for some asset's settlement flows, the Request and the Retargetter are upgraded with longer durations.
- **Donations to the retargetter or its modules are absorbed, not blocked**: dust below the residual tolerance folds into the next operation (a small tolerance, for example `2^20` on a 6-decimal asset, about one token, stops dust donations from griefing settlement); larger amounts are folded back via full-balance rebalance legs; a debt-asset donation exceeding module debt requires the owner to raise the tolerance exponent to settle.
...
- **Unminted mint authorizations survive on the Request.** An authorization not yet minted can still be exercised on the Request directly, so the operator revokes leftovers (a zero-amount `authorizeMinting`, allowed at any time) once the funding round is over; the first pull of funds also revokes every authorization still pending on-chain. A mint front-running that revocation or settlement is bounded by the yield gate and the prorated accrual, and requires the account to have been authorized in the first place.
- **Overstating `order.output` can wedge an order on venues with one-sided deviation checks.** Pareto rejects only outputs below the current rate, so an overstated output commits into an order that never becomes unlockable and stays pending until the fund operator resolves it fund-side. Size `order.output` at or below the venue-simulated output.
...
- **An order can be griefed into perpetual PROCESSING.** An attacker can inflate the vault's `pendingDepositRequest` for the fund via direct `requestDeposit` calls, keeping the original request looking unfilled; `forceEnd()` is the operator escape hatch for such orders once all claimable fills and recoverable cancel assets are drained. For the same reason, `cancelRequest()`'s unclaimed-fills guard is skipped whenever a pending request exists: the claimable amounts may be polluted by attacker deposits and must not block fund operations.
