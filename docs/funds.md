...
A fund processes **one order at a time**. An order is either a DEPOSIT (asset in, wrapped shares out) or a REDEEM (wrapped shares in, asset out) and moves through a state machine driven by the depositor (the Facility); funds pin the order's owner and receiver to the depositor at `create()`.
...
`create()` registers the order, `commit()` moves the value into the external protocol, `unlock()` delivers the proceeds to the receiver, and `recover()` is the failure exit that returns whatever came back. PENDING exists for integrations whose orders queue upstream before acceptance (rate limits, KYC clearing); the current integrations return ACCEPTED directly from `create()`. The async integrations (Centrifuge, Pareto, Midas) store an internal state that can differ from what `state()` reports: `state()` also queries the external protocol to detect transitions that happen off-chain (for example a cleared epoch turning PROCESSING into UNLOCKING).

The depositor can batch transitions in a single transaction when the returned state allows it: `create()` immediately followed by `commit()` when `create()` returns ACCEPTED, or `commit()` followed by `unlock()` for synchronous settlement (the Pareto deposit path).
...
- `resolve()` lets the owner or operator override a PROCESSING deposit order's expected output, which serves as the received-balance threshold; the input argument is only recorded in the event, and redeems ignore resolution entirely.
...
5. Second `commit()` (redeem leg): settles the remainder instantly via `redeemInstant()`. The order's minimum output is enforced on-chain by the vault and re-checked by the fund against the received balance, since redeem outputs are not oracle-validated at create (the per-redemption vault does not exist yet, and its settlement pricing may legitimately differ from the external oracle).
...
- A **redeem** can be aborted in two states: the bond leg has committed with the instant redemption still locked (even when an empty bond config meant nothing was actually paid), or the order was re-armed by `unlockInstantRedeem()` with the bond already paid. An uncommitted redeem is rejected in favor of `cancel()`.
- Aborting a redeem forfeits the bond unless Midas returns it off-band, in mTokens; the remainder shares never left the depositor, so the bond is the only amount possibly owed back. `recover()` re-wraps the returned mTokens 1:1 into shares minted to the receiver (possibly zero when the bond stays forfeited), landing the refund inside the Facility's intent accounting, which snapshots the share token on a redeem recover.
